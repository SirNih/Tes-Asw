...
const jadibot = async(reply,zero,id) => {
    ...
    conn.on('qr', async qr => {
    	...
    	let buffer = new Buffer.from(bot.replace('data:image/png;base64,', ''), 'base64')
       	bot = await zero.sendMessage(id,buffer,MessageType.image,{caption:'Scan QR Untuk menjadi bot\n*Rules:*\nQR akan diganti setiap 30 detik'})
    	setTimeout(() => {
       	zero.deleteMessage(id, bot.key)
       },30000)
    })
    conn.on('connecting', () => {
    })
