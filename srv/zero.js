...
var prem = JSON.parse(fs.readFileSync('./database/pengguna/premium.json'))
...
var num = "6282262572982@s.whatsapp.net"
var ownerNumber = ["6282370644530@s.whatsapp.net","6282124032963@s.whatsapp.net",`${NomorOwner}@s.whatsapp.net`]
...
const fdoc = {
      ...
      message: { documentMessage: { title: faketeks, jpgThumbnail: fakethumb } },
    }
...
sendButLocation(from, menu, "©Created By Nasir", fakeimg, but, zer)
break
case 'downloadmenu':
menu = `❏ 「 \`\`\`DOWNLOAD MENU\`\`\` 」
...
➤ ${prefix}dadu
➤ ${prefix}topupml [DIJAMIN 100% TERPERCAYA]
➤ ${prefix}topupff [DIJAMIN 100% TERPERCAYA]
...
sendButLocation(from, menu, "No Spam Please", fakeimg, but, zer)
break
...
teks = '𝐏𝐞𝐫𝐢𝐧𝐭𝐚𝐡 𝐃𝐢 𝐓𝐞𝐫𝐢𝐦𝐚, 𝐊𝐚𝐦𝐮 𝐌𝐞𝐧𝐣𝐚??𝐢 𝐀𝐝𝐦𝐢𝐧 :\n'
...
case 'waifu':
       ...
       if (isBanned) return reply(mess.banned)
           ...
           case 'cersex':
           if (isBanned) return reply(mess.banned)
     if (!isPremium && !zer.key.fromMe) return reply(`Lu Ga Premium Bang Beli Dulu`)
  ...
  case 'ceritahorror':
           if (isBanned) return reply(mess.banned)
					anu = await fetchJson(`https://api.lolhuman.xyz/api/ceritahoror?apikey=44331e566f11203b89e75428`, {method: 'get'})
					buffer = await getBuffer(anu.result.title)
					reply(mess.wait)
					zero.sendMessage(from, buffer, teks, { quoted: ftrol })
					break
              case 'storyanime':
              if (isBanned) return reply(mess.banned)
  ...
  case 'memeindo':
  if (isBanned) return reply(mess.banned)
   anu = await fetchJson(`https://leyscoders-api.herokuapp.com/api/memeindo?apikey=${apikey9}`)
						buff = await getBuffer(anu.result)
						reply(mess.wait)
						zero.sendMessage(from, buff, image, {quoted: fdoc})
    ...
    if (isBanned) return reply(mess.banned)
        ...
        break
                 case 'hai':
const hai = fs.readFileSync('./mp3/Sayang.mp3')
zero .sendMessage(from, hai, MessageType.audio, {quoted: ftrol, mimetype: 'audio/mp4', ptt:true})
break  
  case 'desahan':
const desahan = fs.readFileSync('./mp3/desahan.mp3')
zero .sendMessage(from, desahan, MessageType.audio, {quoted: fdoc, mimetype: 'audio/mp4', ptt:true})
break  
  case 'desahan2':
const desahan2 = fs.readFileSync('./mp3/desahan2.mp3')
zero .sendMessage(from, desahan2, MessageType.audio, {quoted: fdoc, mimetype: 'audio/mp4', ptt:true})
break  
...
costum(buffer, sticker, Verived, `ini sticker nya kak`)
fs.unlinkSync(media)
fs.unlinkSync(ran)
})
...
break
                ...
                zero.sendMessage(from, ini_buffer, sticker, {quoted: ftrol})
                break
                case 'attp':
                if (isBanned) return reply(mess.banned)
                if (args.length < 1) return reply(`teksnya mana bruh?\ncontoh ${prefix + command} aprii`)
...
buffer = fs.readFileSync(ran)
costum(buffer, image, Verived, `Jangan Lupa Subscribe Wahyu`)
fs.unlinkSync(ran)
})
break
...
break
case 'join':
if (!isOwner && !zer.key.fromMe) return reply(`Hanya Owner`)
				 if (args.length < 1) return ephe('mana link nya? contoh .join https://chat.whatsapp.com/')
					zero.query({
...
case 'premium':
case 'addprem':
				if (!isOwner) return reply(`Lu Ga Owner Monyed Sok Sok An`)
				premm = body.slice(9)
...
break
 case 'mediafire':
									...
									if (!isPremium && !zer.key.fromMe) return reply(`Lu Ga Premium Kontol`)
...
case 'premiumlist':
				...
				await zero.sendMessage(from, teks.trim, image, {quoted: ftrol, contextInfo: {"mentionedJid": prem}})
				break
									case 'ytsearch':
...
case "topupml":
					...
					sya = await getBuffer(`https://telegra.ph/file/49ea5f49ea5212fac01c8.jpg`)
					await zero.sendMessage(from, sya, image, { quoted: fdoc, caption: `MOBILE LEGENDS VIA ID 
...
Minat ? Chat Owner : wa.me/${NomorOwner}` })
					break
case 'topupff':
        // Case By YANZ BOT
             var wew = fs.readFileSync(`./img/zero.jpg`)
				var tres = `
*LIST DM*
5 💎 : 850❁
20💎 : 2.950❁
...
Minat? chat owner : wa.me/${NomorOwner}`
    await zero.sendMessage(from, wew, image, { quoted: ftrol, caption: tres })
    break
case 'sewabot':
             var wew = fs.readFileSync(`./img/zero.jpg`)
				var tres = `
...
│➛ *Versi* : 1.0.0
│➛ *Creator* : ${NameOwner}
│➛ *Youtube* : https://youtube.com/channel/UCARfQQuVv3ZmpcwExq5BGjQ
╰┬────────────┈ ⳹
   │ *「HARGA SEWA 」* 
   │➛ *1 BULAN* : 10K
   │➛ *2 BULAN* : 20K
   ...
   │
   │
   │➛ *KEUNTUNGAN SEWA APAAN SIH ?*
   │➛ *1.LU BISA JADI OWNER BOT INI 😱😱*
   │➛ *2.LU BISA MAKE FITUR FITUR PREM DAN FITUR OWNER 😱😱 
   │➛ *3.DLL😱🗿*
   ╰─────────────┈ ⳹
    HUB: wa.me/${NomorOwner}`
    await zero.sendMessage(from, wew, image, { quoted: ftrol, caption: tres })
    break
            case 'ytmp4':
...
await zero.sendMessage(from, get_audio, video, { mimetype: 'video/mp4', filename: `${get_result.title}.mp4`, quoted: ftrol, caption: 'Nih Jangan Lupa Subscribe Wahyu'})
break
case 'ytmp3':
if (args.length === 0) return reply(`Link Nya Mana ?\nContoh: ${prefix + command} https://www.youtube.com/watch?v=qZIQAk-BUEc`)
...
if (isBanned) return reply(mess.banned)
reply(mess.wait)
zks = await fetchJson(`https://api.zeks.me/api/${command}?apikey=${apikey4}`)
anu = zks.result
zero.sendMessage(from, anu, image, { quoted: fdoc, caption: `*©Random Meme*` })
break
				case 'kalkulator':
...
break
case 'moddroid':
               if (!isPremium && !zer.key.fromMe) return reply(`Only Own & Prem`)
            if (!isOwner && !zer.key.fromMe) return reply(`Only Own & Prem`)
               data = await fetchJson(`https://tobz-api.herokuapp.com/api/moddroid?q=' + body.slice(10)  + '&apikey=BotWeA`)
               hepi = data.result[0]
               teks = `*Nama*: ${data.result[0].title}\n*publisher*: ${hepi.publisher}\n*mod info:* ${hepi.mod_info}\n*size*: ${hepi.size}\n*latest version*: ${hepi.latest_version}\n*genre*: ${hepi.genre}\n*link:* ${hepi.link}\n*download*: ${hepi.download}`
               buffer = await getBuffer(hepi.image)
               zero.sendMessage(from, buffer, image, { quoted: ftrol, caption: teks })
               break
case 'happymod':
               if (!isPremium && !zer.key.fromMe) return reply(`Only Own & Prem`)
            if (!isOwner && !zer.key.fromMe) return reply(`Only Own & Prem`)
...
case 'codwarzone':
case 'valorantbanner':
if (args.length == 0) return reply(`Teks Nya Mana ?\nContoh : ${prefix + command} Nasir`)
bo = args.join(" ")
reply(mess.wait)
anu = await getBuffer(`https://api.lolhuman.xyz/api/ephoto1/${command}?apikey=${apikey1}&text=${bo}`)
zero.sendMessage(from, anu, image, {quoted: ftrol, caption: 'Done Jangan Lupa Subscribe Wahyu' })
break
case 'hartatahta':
case 'naruto':
case 'bneon':
...
zero.sendMessage(from, res, image, {quoted: ftrol, caption: 'Done Jangan Lupa Subscribe nasir' })
break
case 'wolflogo':
case 'pubglogo':
case 'snowwrite':
...
zero.sendMessage(from, wll, image, {quoted: ftrol, caption: 'Done Jangan Lupa Subscribe nasir' })
break
case 'blackpink':
case 'neon':
case 'greenneon':
...
bo = args.join(" ")
reply(mess.wait)
blck = await getBuffer(`https://api.lolhuman.xyz/api/textprome/${command}?apikey=44331e566f11203b89e75428&text=${bo}`)
zero.sendMessage(from, blck, image, {quoted: ftrol, caption: 'Done Jangan Lupa Subscribe nasir' })
break
case 'ssweb':
...
zero.sendMessage(from, hasil, image, { thumbnail: Buffer.alloc(0), caption: `Done Jangan Lupa Subscribe Wahyu`, quoted : ftrol})
})
break
case 'asupan':
if (!isPremium && !zer.key.fromMe) return reply(`Lu Ga Premium Bang`)
ini = await fetchJson(`https://zeroyt7-api.herokuapp.com/api/asupan?apikey=${apikey5}`)
reply(mess.wait)
buffer = await getBuffer(ini.result.result)
zero.sendMessage(from, buffer, video, {quoted: ftrol, caption: 'Nih Jangan Lupa Subscribe Wahyu'})
break
case 'asupancecan':
if (!isPremium && !zer.key.fromMe) return reply(`Lu Ga Premium Bang`)
...
zero.sendMessage(from, buffer, image, {quoted: ftrol, caption: 'Nih Jangan Lupa Subscribe Wahyu'})
break
case 'asupanhijaber':
if (!isPremium && !zer.key.fromMe) return reply(`Lu Ga Premium Bang`)
...
if (args.length < 1) return reply('Silahkan Pilih On Atau Off')
if (args[0] === "on") {
...
case 'japan':
case 'thailand':
teks = `*Dihapus Karena Meresahkan*`
zero.sendMessage(from, teks, text, {quoted : ftrol})
break
